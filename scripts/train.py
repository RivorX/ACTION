...
def objective(trial, train_dataset: TimeSeriesDataSet, val_dataset: TimeSeriesDataSet, config: dict):
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            EarlyStopping(monitor=monitor_metric, patience=config['training']['early_stopping_patience'], mode="min"),
            CustomModelCheckpoint(monitor=monitor_metric, save_path=config['paths']['model_save_path'], mode="min")
        ],
